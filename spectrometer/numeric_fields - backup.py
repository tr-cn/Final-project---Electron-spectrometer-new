...
class MagneticField:
    ...
    def _get_magnetic_field(self,R_current_m):
        ...
        x_ind = np.argmin(np.abs(self.x - R_current_m[0]))
        y_ind = np.argmin(np.abs(self.y - R_current_m[1]))
        
        x0 = self.x[x_ind]; x1 = self.x[x_ind+1];
        y0 = self.y[y_ind]; y1 = self.x[y_ind+1]
        
        frac_x = (x_c-x0) / (x1 - x0) 
        frac_y = (y_c-y0) / (y1 - y0) 
        
        
        Bx_p = self.Bx[x_ind] + self.Bx[x_ind] * frac_x * (self.Bx[x_ind+1] - self.Bx[x_ind])
        By_p = self.By[x_ind] + self.By[x_ind] * frac_y * (self.By[x_ind+1] - self.By[x_ind])
        
        Bx_cell = self.Bx[x_ind,y_ind]
        By_cell = self.Bx[x_ind,y_ind]
