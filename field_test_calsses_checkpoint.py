...
class Potential:
    ...
    def laplace_residuals (self, potential):
        dz = self.dz_current;  dy = self.dy_current;
    ...
    def restrict_potential(self, potential):
        ...
        r = []; e = []
        self.dz_current = self.dz; self.dy_current = self.dy;
        r.append(self.laplace_residuals(potential))
        while Nz>3 or Ny>3:
            r_finer = r[-1]
            r_coarser = self.restrict_residual(r_finer)
            self.dz_current = self.dz_current*2; self.dy_current = self.dy_current*2; 
...
if __name__ == "__main__":
    ...
    phi = ElectricPotential(Nz=2**4+1, Ny=2**4+1, Lz_mm=50.8, Ly_mm=12.5, E0_Vm=10,
    ...
    if True:
        psi = MagneticPotential(Nz=2**4+1, Ny=2**4+1, Lz_mm=50.8, Ly_mm=12.5, B0_T=10,
                                pole_z_start_mm=-10.0, pole_z_end_mm=10.0)
